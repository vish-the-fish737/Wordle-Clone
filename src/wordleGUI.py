...
def main():
    global current_guess, game_ended  # Add game_ended to the global variables
    guessed_letters = {'correct': set(), 'present': set(), 'absent': set()} 
    run = True
    game_ended = False  # Initialize game_ended variable
    while run:
        SCREEN.fill(DARK_MODE_BLACK)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if game_ended and restart_rect.collidepoint(event.pos):
                    resetBoard()

        if game_ended:
            if restart_rect is None:
                restart_rect = displayEndMessage("Game Over! The word was: {}".format(correct_word))
            else:
                resetBoard()
                draw_components()
                draw_keyboard(guessed_letters)
